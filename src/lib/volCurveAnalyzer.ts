/**
 * Vol Curve Analyzer (library)
 *
 * Pure analysis library: given thesis parameters and a ticker, fetches the
 * options chain, computes vol smile / edge ratios / term structure / IV-vs-RV
 * history, and ranks candidate strategies. Returns a self-contained
 * AnalysisOutput object — caller is responsible for env loading, persistence,
 * and process lifecycle.
 *
 * Refactored from scripts/vol-curve-analyze.ts (Phase 2). The CLI script is
 * now a thin wrapper around analyzeTicker(); the same library powers
 * scanner-triggered analyses via /api/vol-curve/analyze-snapshot/[id].
 *
 * Data sources (priority): live Massive API → options_chain_snapshots DB →
 * underlyings_iv_history spot fallback.
 */
...
export interface AnalyzeOptions {
  ...
  regime?: 'cheap' | 'rich' | 'mixed' | 'neutral';
  useCase?: string;
...
function resolveArgs(opts: AnalyzeOptions): ResolvedArgs {
  if (
    !opts.ticker ||
    !opts.direction ||
    !Number.isFinite(opts.targetBase) ||
    !Number.isFinite(opts.targetHigh) ||
    !Number.isFinite(opts.horizonMonths) ||
    !Number.isFinite(opts.downsideFloor)
  ) {
    throw new Error(
      'analyzeTicker: ticker, direction, targetBase, targetHigh, horizonMonths, downsideFloor are required'
    );
  }
  return {
    ...
    horizonRange: opts.horizonRange ?? 2,
    downsideFloor: opts.downsideFloor,
    riskFreeRate: opts.riskFreeRate ?? 0.045,
    snapshotDate: opts.snapshotDate ?? null,
    regime: opts.regime ?? 'neutral',
    useCase: opts.useCase ?? null,
    hasOpenPosition: opts.hasOpenPosition ?? false,
  };
}
...
async function fetchLiveChain(
  ...
  while (nextUrl) {
    try {
      ...
      const data: { results?: MassiveOptionResult[]; next_url?: string } = await resp.json();
...
function computeBreakeven(legs: Leg[], spot: number, direction: 'bullish' | 'bearish' | 'neutral'): number {
...
function generateStrategies(
  ...
  // ===================== PHASE 2: BEARISH STRATEGIES =====================

  if (args.direction === 'bearish') {
    // Long put candidates: edge ratio sorted, below spot
    const allPuts = computeEdgeRatios(
      expiry.puts, spot, T, r, args.targetBase, thesisSigma, 'put'
    ).sort((a, b) => b.edgeRatio - a.edgeRatio);

    const longPutCandidates = allPuts.filter(p => p.strike < spot).slice(0, 3);

    // Short put candidates: between targetBase and downsideFloor (sells the floor)
    const shortPutForSpread = allPuts
      .filter(p => p.strike >= args.downsideFloor && p.strike <= args.targetBase)
      .sort((a, b) => b.strike - a.strike);

    // Short call candidates for bearish seagull: above spot, fund the put
    const shortCallsForFund = callEdges
      .filter(c => c.strike >= spot * 1.05)
      .sort((a, b) => b.marketPrice - a.marketPrice)
      .slice(0, 3);

    for (const longPut of longPutCandidates) {
      const longLeg = makeLeg('buy', longPut, spot, T, r);
      ...
      // === NAKED PUT ===
      {
        ...
        const netDebit = longLeg.marketPrice;
        const payoffBase = evaluatePayoffAtPrice(legs, args.targetBase);
        const payoffHigh = evaluatePayoffAtPrice(legs, args.targetHigh); // bearish: targetHigh = lower
        const maxLoss = netDebit;
        const maxProfit = evaluatePayoffAtPrice(legs, 0.01);
        const breakeven = computeBreakeven(legs, spot, args.direction);
        const rorBase = maxLoss > 0 ? payoffBase / maxLoss : 0;
        const rorHigh = maxLoss > 0 ? payoffHigh / maxLoss : 0;
        ...
        strategies.push({
          rank: 0,
          label: `${expiry.expiry.slice(0, 10)} ${longPut.strike}P`,
          type: 'naked_put',
          expiry: expiry.expiry,
          dte: expiry.dte,
          legs,
          ...
          netVega: longLeg.vega,
          netTheta: longLeg.theta,
          avgEdgeRatio: longPut.edgeRatio,
          liquidityScore: Math.min(1, (longPut.openInterest + longPut.volume) / 1000),
        });
      }

      // === PUT SPREADS (bear debit) ===
      for (const shortPut of shortPutForSpread) {
        if (shortPut.strike >= longPut.strike) continue;

        const shortLeg = makeLeg('sell', shortPut, spot, T, r);
        const legs = [longLeg, shortLeg];
        ...
        const rorBase = maxLoss > 0 ? payoffBase / maxLoss : 0;
        const rorHigh = maxLoss > 0 ? payoffHigh / maxLoss : 0;
        const avgEdge = (longPut.edgeRatio + (1 / Math.max(shortPut.edgeRatio, 0.01))) / 2;
        const minOi = Math.min(longPut.openInterest, shortPut.openInterest);

        strategies.push({
          rank: 0,
          label: `${expiry.expiry.slice(0, 10)} ${longPut.strike}/${shortPut.strike}P`,
          type: 'put_spread',
          expiry: expiry.expiry,
          dte: expiry.dte,
          legs,
          ...
          netTheta: longLeg.theta + shortLeg.theta,
          avgEdgeRatio: avgEdge,
          liquidityScore: Math.min(1, (minOi + Math.min(longPut.volume, shortPut.volume)) / 1000),
        });
      }

      // === SEAGULL (bearish: long put + short put cap + short call fund) ===
      for (const shortPut of shortPutForSpread) {
        if (shortPut.strike >= longPut.strike) continue;
        for (const shortCall of shortCallsForFund) {
          const shortPutLeg = makeLeg('sell', shortPut, spot, T, r);
          const shortCallLeg = makeLeg('sell', shortCall, spot, T, r);
          const legs = [longLeg, shortPutLeg, shortCallLeg];

          const netDebit =
            longLeg.marketPrice - shortPutLeg.marketPrice - shortCallLeg.marketPrice;
          const spreadWidth = longPut.strike - shortPut.strike;

          // Max profit at price = short put strike (downside cap)
          const maxProfit = spreadWidth - Math.max(netDebit, 0);
          const upsideLoss = shortCall.strike - spot + Math.max(netDebit, 0);
          const maxLoss = Math.max(Math.max(netDebit, 0), upsideLoss);

          const payoffBase = evaluatePayoffAtPrice(legs, args.targetBase);
          const payoffHigh = evaluatePayoffAtPrice(legs, args.targetHigh);
          const breakeven = computeBreakeven(legs, spot, args.direction);

          const callMarginEstimate = shortCall.strike * 0.2;
          const capitalAtRisk = Math.max(netDebit, callMarginEstimate);
          const rorBase = capitalAtRisk > 0 ? payoffBase / capitalAtRisk : 0;
          const rorHigh = capitalAtRisk > 0 ? payoffHigh / capitalAtRisk : 0;
          ...
          strategies.push({
            rank: 0,
            label: `${expiry.expiry.slice(0, 10)} ${longPut.strike}/${shortPut.strike}P, -${shortCall.strike}C [seagull]`,
            type: 'seagull',
            expiry: expiry.expiry,
            dte: expiry.dte,
            legs,
            ...
            annualizedRorBase: rorBase * (365 / expiry.dte),
            annualizedRorHigh: rorHigh * (365 / expiry.dte),
            netDelta: longLeg.delta + shortPutLeg.delta + shortCallLeg.delta,
            netVega: longLeg.vega + shortPutLeg.vega + shortCallLeg.vega,
            netTheta: longLeg.theta + shortPutLeg.theta + shortCallLeg.theta,
            avgEdgeRatio: (longPut.edgeRatio + shortCall.edgeRatio) / 2,
            liquidityScore: Math.min(1, minOi / 500),
          });
        }
      }
  ...
  // ===================== PHASE 2: SHORT-VOL / RICH-REGIME STRATEGIES =====================

  const allCallsByEdge = callEdges.sort((a, b) => a.strike - b.strike);
  const allPutsByEdge = computeEdgeRatios(
    expiry.puts, spot, T, r, args.targetBase, thesisSigma, 'put'
  ).sort((a, b) => a.strike - b.strike);

  // === COVERED CALL ===
  // Only generate when user already holds the underlying.
  // Short an OTM call near targetHigh — caps upside in exchange for premium.
  if (args.hasOpenPosition && args.regime !== 'cheap') {
    const ccCandidates = allCallsByEdge
      .filter(c => c.strike >= spot * 1.02 && c.strike <= args.targetHigh * 1.1)
      .filter(c => c.marketPrice >= spot * 0.005)
      .sort((a, b) => b.marketPrice / b.strike - a.marketPrice / a.strike) // best yield-per-strike
      .slice(0, 3);

    for (const cc of ccCandidates) {
      const shortLeg = makeLeg('sell', cc, spot, T, r);
      const legs = [shortLeg];
      const netCredit = shortLeg.marketPrice;
      const netDebit = -netCredit; // negative debit = credit

      // For a covered call, max profit = call premium + (strike - spot) [if called away]
      const maxProfit = netCredit + Math.max(cc.strike - spot, 0);
      // Max loss is unbounded on the underlying side; report just the break-even helper
      const maxLoss = spot - netCredit; // approximation: stock to zero, less premium received
      const breakeven = spot - netCredit;

      // Payoff terms relative to spot (since user already owns the stock)
      const payoffBase = evaluatePayoffAtPrice(legs, args.targetBase);
      const payoffHigh = evaluatePayoffAtPrice(legs, args.targetHigh);

      // Capital at risk for ROR: stock cost basis (just use spot as proxy)
      const rorBase = spot > 0 ? netCredit / spot : 0;
      const rorHigh = spot > 0 ? netCredit / spot : 0;

      strategies.push({
        rank: 0,
        label: `${expiry.expiry.slice(0, 10)} -${cc.strike}C [covered]`,
        type: 'covered_call',
        expiry: expiry.expiry,
        dte: expiry.dte,
        legs,
        ...
        annualizedRorBase: rorBase * (365 / expiry.dte),
        annualizedRorHigh: rorHigh * (365 / expiry.dte),
        netDelta: shortLeg.delta + 1, // +1 for the long stock leg
        netVega: shortLeg.vega,
        netTheta: shortLeg.theta,
        avgEdgeRatio: 1 / Math.max(cc.edgeRatio, 0.01), // inverted: selling
        liquidityScore: Math.min(1, (cc.openInterest + cc.volume) / 1000),
      });
    }
  }

  // === CASH-SECURED PUT ===
  // Short OTM put — get paid to wait for entry at lower price.
  // Always available (rich regime preferred but works in mixed too).
  if (args.regime === 'rich' || args.regime === 'mixed' || args.regime === 'cheap') {
    const cspCandidates = allPutsByEdge
      .filter(p => p.strike < spot && p.strike >= args.downsideFloor * 0.95)
      .filter(p => p.marketPrice >= spot * 0.005)
      .sort((a, b) => b.marketPrice / b.strike - a.marketPrice / a.strike)
      .slice(0, 3);

    for (const csp of cspCandidates) {
      const shortLeg = makeLeg('sell', csp, spot, T, r);
      const legs = [shortLeg];
      const netCredit = shortLeg.marketPrice;
      const netDebit = -netCredit;
      // Max profit = premium kept; max loss = strike - premium (assignment at strike)
      const maxProfit = netCredit;
      const maxLoss = csp.strike - netCredit;
      const breakeven = csp.strike - netCredit;

      const payoffBase = evaluatePayoffAtPrice(legs, args.targetBase);
      const payoffHigh = evaluatePayoffAtPrice(legs, args.targetHigh);
      // Capital at risk = strike (cash held to secure assignment)
      const capitalAtRisk = csp.strike;
      const rorBase = capitalAtRisk > 0 ? netCredit / capitalAtRisk : 0;
      const rorHigh = capitalAtRisk > 0 ? netCredit / capitalAtRisk : 0;

      strategies.push({
        rank: 0,
        label: `${expiry.expiry.slice(0, 10)} -${csp.strike}P [CSP]`,
        ...
        annualizedRorBase: rorBase * (365 / expiry.dte),
        annualizedRorHigh: rorHigh * (365 / expiry.dte),
        netDelta: shortLeg.delta,
        netVega: shortLeg.vega,
        netTheta: shortLeg.theta,
        avgEdgeRatio: 1 / Math.max(csp.edgeRatio, 0.01),
        liquidityScore: Math.min(1, (csp.openInterest + csp.volume) / 1000),
      });
    }
  }

  // === IRON CONDOR ===
  // Short OTM call/put with long further-OTM wings on both sides — collects
  // premium with capped risk; benefits from low realized vol and time decay.
  if (args.regime === 'rich') {
    // Short call ~5-15% above spot; short put ~5-15% below
    const shortCallCandidates = allCallsByEdge
      .filter(c => c.strike >= spot * 1.05 && c.strike <= spot * 1.20)
      .filter(c => c.marketPrice >= spot * 0.003);
    const shortPutCandidates = allPutsByEdge
      .filter(p => p.strike <= spot * 0.95 && p.strike >= spot * 0.80)
      .filter(p => p.marketPrice >= spot * 0.003);

    for (const sc of shortCallCandidates.slice(0, 2)) {
      // Find matching long call wing ~5% above short
      const lc = allCallsByEdge.find(c => c.strike >= sc.strike * 1.04 && c.strike <= sc.strike * 1.10);
      if (!lc) continue;

      for (const sp of shortPutCandidates.slice(0, 2)) {
        const lp = allPutsByEdge.slice().reverse().find(p => p.strike <= sp.strike * 0.96 && p.strike >= sp.strike * 0.90);
        if (!lp) continue;

        const shortCallLeg = makeLeg('sell', sc, spot, T, r);
        const longCallLeg = makeLeg('buy', lc, spot, T, r);
        const shortPutLeg = makeLeg('sell', sp, spot, T, r);
        const longPutLeg = makeLeg('buy', lp, spot, T, r);
        const legs = [shortCallLeg, longCallLeg, shortPutLeg, longPutLeg];

        const netCredit =
          shortCallLeg.marketPrice + shortPutLeg.marketPrice -
          longCallLeg.marketPrice - longPutLeg.marketPrice;
        if (netCredit <= 0) continue; // Should always be a net credit
        const netDebit = -netCredit;
        ...
        // Two breakevens; report the upper one as primary
        const breakeven = sc.strike + netCredit;

        const payoffBase = evaluatePayoffAtPrice(legs, args.targetBase);
        const payoffHigh = evaluatePayoffAtPrice(legs, args.targetHigh);
        const rorBase = maxLoss > 0 ? netCredit / maxLoss : 0;
        const rorHigh = maxLoss > 0 ? netCredit / maxLoss : 0;
        const minOi = Math.min(sc.openInterest, lc.openInterest, sp.openInterest, lp.openInterest);

        strategies.push({
          rank: 0,
          label: `${expiry.expiry.slice(0, 10)} IC ${lp.strike}/${sp.strike}P, ${sc.strike}/${lc.strike}C`,
          type: 'iron_condor',
          expiry: expiry.expiry,
          dte: expiry.dte,
          legs,
          ...
          annualizedRorBase: rorBase * (365 / expiry.dte),
          annualizedRorHigh: rorHigh * (365 / expiry.dte),
          netDelta: shortCallLeg.delta + longCallLeg.delta + shortPutLeg.delta + longPutLeg.delta,
          netVega: shortCallLeg.vega + longCallLeg.vega + shortPutLeg.vega + longPutLeg.vega,
          netTheta: shortCallLeg.theta + longCallLeg.theta + shortPutLeg.theta + longPutLeg.theta,
          avgEdgeRatio: 1.0, // Neutral — premium capture, not directional edge
          liquidityScore: Math.min(1, minOi / 500),
        });
      }
    }
...
export async function analyzeTicker(opts: AnalyzeOptions): Promise<AnalysisOutput> {
  ...
  if (!spot || spot === 0) {
    throw new Error(`Could not determine spot price for ${args.ticker}`);
  }

  if (chainData.rows.length === 0) {
    throw new Error(
      `No options chain data available for ${args.ticker} in DTE range ${minDte}-${maxDte}. Try a shorter horizon or check Massive API key.`
    );
  }
  ...
  // Phase 2: quotas widened to cover bearish + rich-regime strategies.
  // The total budget allows up to ~20 — generous because user filters in UI.
  const topN = 20;
  const selectedStrategies: Strategy[] = [];
  const typeQuota: Record<string, number> = {
    naked_call: 2, call_spread: 3, risk_reversal: 5, butterfly: 2,
    naked_put: 2, put_spread: 3, put_butterfly: 2, seagull: 3,
    covered_call: 2, cash_secured_put: 2, iron_condor: 2,
  };
  const typeCounts: Record<string, number> = {};
  Object.keys(typeQuota).forEach(k => { typeCounts[k] = 0; });

  // First pass: fill from ranked list
  for (const s of ranked) {
    if (selectedStrategies.length >= topN) break;
    if ((typeCounts[s.type] ?? 0) < (typeQuota[s.type] ?? 0)) {
      selectedStrategies.push(s);
      typeCounts[s.type] = (typeCounts[s.type] ?? 0) + 1;
    }
  }

  // Second pass: if any type has 0 entries but the user's regime/direction
  // makes it relevant, add the best of that type.
  const relevantTypes: Strategy['type'][] = (() => {
    if (args.direction === 'bearish') {
      return ['naked_put', 'put_spread', 'seagull', 'cash_secured_put', 'iron_condor'];
    }
    if (args.regime === 'rich') {
      return ['covered_call', 'cash_secured_put', 'iron_condor', 'call_spread'];
    }
    return ['naked_call', 'call_spread', 'risk_reversal', 'butterfly'];
  })();
