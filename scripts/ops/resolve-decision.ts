 ...
 *
 * Usage:
 *   npx tsx scripts/ops/resolve-decision.ts --id <decisionId> --action set_gated_by --macro-id <uuid> --by user
 *   npx tsx scripts/ops/resolve-decision.ts --id <decisionId> --action dismiss --by user
 *   npx tsx scripts/ops/resolve-decision.ts --id <decisionId> --action <selected-action> --by user \
...
async function readInput(): Promise<Input> {
  ...
  return {
    ...
    writes: a.writes
      ? (JSON.parse(a.writes as string) as GraphWrite[])
      : undefined,
...
async function runHandler(
  ...
  if (type === "classify_macro_link" || type === "frame_asset_under_macro") {
    ...
    if (action === "unlink") {
      ...
      return [
        {
          table: "asset_thesis_related_macro_theses",
          op: "delete",
          ids: [assetThesisId, macroThesisId],
        },
      ];
    }
    ...
    if (!input.dryRun) {
      await linkAssetMacro({
        ...
        macroThesisId,
        relationshipType,
        addedBy: input.by === "user" ? "user" : "automation",
      });
    }
    return [
      {
  ...
  if (type === "link_strategy_to_thesis" && action === "link") {
    ...
    if (!strategyId || !thesisId)
      throw new Error(
        "link_strategy_to_thesis needs strategyId (primary or --strategy-id) and --thesis-id (asset)",
      );
    if (!input.dryRun) {
      await db
        .update(strategies)
        .set({ assetThesisId: thesisId })
  ...
  if (type === "resolve_proxy_underlying" && action === "map") {
    ...
    if (!underlyingId || !input.parentId)
      throw new Error(
        "resolve_proxy_underlying needs --underlying-id (or a related underlying) and --parent-id",
      );
    if (!input.dryRun) {
      await db
        .update(underlyings)
        .set({ parentUnderlyingId: input.parentId })
  ...
  if (type === "confirm_claim_link" && action === "sever") {
    ...
    const sourceClaimId =
      (typeof meta.sourceClaimId === "string"
        ? meta.sourceClaimId
        : undefined) ??
      (typeof evidence.sourceClaimId === "string"
        ? evidence.sourceClaimId
        : undefined);
    if (!insightId || !sourceClaimId) {
      throw new Error(
        "confirm_claim_link/sever needs metadata.insightId and metadata.sourceClaimId",
      );
    }
    if (
      row.objectType !== "macro_thesis" &&
      row.objectType !== "asset_thesis"
    ) {
      throw new Error(
        "confirm_claim_link/sever primary object must be a macro_thesis or asset_thesis",
      );
    }
    ...
    if (!claim) {
      throw new Error(
        `No main_claims row found for insight/source claim ${insightId}/${sourceClaimId}`,
      );
    }
    ...
    if (!mapping) {
      throw new Error(
        `No claim_thesis_mappings row found for claim ${claim.id} and ${row.objectType} ${row.objectId}`,
      );
    }
    ...
    if (!input.dryRun) {
      await db
        ...
        .where(eq(claimThesisMappings.id, mapping.id));
    }
    return [
      { table: "claim_thesis_mappings", op: "delete", ids: [mapping.id] },
    ];
  }
  ...
  if (type === "cluster_claims_to_thesis" && action === "create_macro") {
    ...
    if (!macroId)
      throw new Error(
        "cluster_claims_to_thesis/create_macro needs --macro-id (the created macro)",
      );
    const members =
      (packet?.evidence_context?.memberAssetThesisIds as
        string[] | undefined) ??
      packet?.related_objects
        ?.filter((o) => o.type === "asset_thesis")
        .map((o) => o.id) ??
      [];
    if (!input.dryRun) {
      for (const assetId of members) {
        await linkAssetMacro({
          assetThesisId: assetId,
          macroThesisId: macroId,
...
async function main() {
  ...
  assertValidDecisionResolutionRequest(input);
  ...
  if (rows.length === 0) {
    console.error(
      `No open decision_required journal entry with id ${input.id}`,
    );
    process.exit(1);
  }
  const row = rows[0] as DecisionRow;
  const packet = getDecisionPacket(row.metadata);
  assertBoundedDecisionSelection(input, packet);
      ...
      JSON.stringify(
        {
          dryRun: true,
          id: input.id,
          decisionType: packet?.decision_type ?? null,
          status,
          resolution,
        },
        null,
