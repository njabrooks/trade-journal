...
async function main() {
  ...
  try {
    ...
    const legacyCount = await db.execute<{ count: string }>(
      sql`SELECT COUNT(*) as count FROM triage_records WHERE recommended_action = 'PROVIDE_STRATEGY_METADATA'`
    );
    const legacyTotal = parseInt(legacyCount.rows[0]?.count || '0', 10);
    ...
    const nullSeverityCount = await db.execute<{ count: string }>(
      sql`SELECT COUNT(*) as count FROM triage_records WHERE severity IS NULL AND recommended_action = 'QUANTITY_CHANGE'`
    );
    const nullTotal = parseInt(nullSeverityCount.rows[0]?.count || '0', 10);
    ...
    const missingConfirm = await db.execute<{ id: string; strategy_key: string; created_at: string }>(
      sql`
    ...
    console.log(`\n[P0.3] Unconfirmed auto-strategies WITHOUT CONFIRM_STRATEGY triage: ${missingConfirm.rows.length}`);
    for (const row of missingConfirm.rows) {
    ...
    const oldDoneCount = await db.execute<{ count: string }>(
      sql`SELECT COUNT(*) as count FROM triage_records WHERE status = 'done' AND updated_at < ${cutoffStr}::timestamptz`
    );
    const oldDoneTotal = parseInt(oldDoneCount.rows[0]?.count || '0', 10);
    ...
    const inboxCount = await db.execute<{ count: string }>(
      sql`SELECT COUNT(*) as count FROM triage_records WHERE status = 'inbox'`
    );
    console.log(`\n--- Summary ---`);
    console.log(`Total triage records remaining: ${totalCount.rows[0]?.count}`);
    console.log(`Inbox records: ${inboxCount.rows[0]?.count}`);
    ...
    console.log(JSON.stringify({
      ...
      missingConfirmStrategy: missingConfirm.rows.length,
