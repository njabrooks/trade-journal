...
async function main() {
  ...
  try {
    ...
    await db.select().from(flexQueryConfigs).limit(1);
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('DATABASE_URL')) {
        console.error('\n   Make sure DATABASE_URL_POOLER is set correctly in your secrets.');
      } else if (error.message.includes('connection') || error.message.includes('timeout')) {
        console.error('\n   Connection issue. Check:');
        console.error('   - DATABASE_URL_POOLER format is correct');
        console.error('   - Database is accessible from GitHub Actions');
        console.error('   - Connection string uses Transaction pooler (port 6543)');
      }
    } else {
