...
Provider Adapter `belief-evidence-assessment-claude` (current):
...
Provider Adapter `belief-research-relation-claude` (current):

## Claude Provider Adapter

1. Require one explicit Trade Journal research insight ID. From the Trade Journal root, run `npx tsx scripts/belief-research-relation.ts --prepare --insight-id <uuid>`. Continue only when it returns `status: ready`; otherwise return the exact unavailable result. Bind every recommendation to the exact `contextDigest`, Notes source/content digest, Toulmin evidence, claim resolutions, developing and monitoring thesis argument digests, and existing relationships.
...
8. Validate the token without writes by running `npx tsx scripts/belief-research-relation.ts --validate-authorization <file|-> --recording <file>`. Validation is readiness only. On missing user acceptance, malformed or expired authority, stale bytes, unavailable input, or user refusal, stop with no writes.
...
Provider Adapter `claims-synthesis-claude` (current):
...
Provider Adapter `decision-resolution-claude` (current):
...
Provider Adapter `portfolio-snapshot-claude` (current):
...
Provider Adapter `research-pipeline-claude` (current):

## Claude Provider Adapter

1. Require one explicit Trade Journal research insight UUID. From the Trade Journal root, use Claude's repository command runner with `npx tsx scripts/research-pipeline.ts`. Build independently invocable zero-write results through `--pipeline-status`, `--idea-intake`, `--thesis-formalization`, `--unknown-mapping`, `--research-preparation`, `--unknown-research`, `--evidence-synthesis`, `--thesis-expression`, `--gate-decision`, or `--graduation` with `<file|->`; validate stage bytes through `--validate-stage-result`. Use `--describe`, `--evaluate`, and `--validate-result` for the aggregate.
...
Provider Adapter `research-publication-claude` (current):
...
2. From the Trade Journal root, run `npx tsx scripts/research-publication.ts --prepare --insight-id <uuid> --synthesis-result <file|->`. Continue only when it returns `status: authorization_required`; otherwise return the exact unavailable result. Bind all work to its `publicationDigest`, current Notes source digest, claims-synthesis result digest, candidates, exclusions, and exact permitted write surface.
...
7. Validate the token without writes by running `npx tsx scripts/research-publication.ts --validate-authorization <file|-> --publication <file>`. Validation means authorization readiness only. On missing user acceptance, malformed or expired authority, stale bytes, unavailable input, or user refusal, stop with no writes.
...
Provider Adapter `thesis-foreground-claude` (current):
...
Provider Adapter `thesis-underwriting-claude` (current):
