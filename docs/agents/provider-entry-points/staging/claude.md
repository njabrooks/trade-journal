...
- The recorder uses one serializable transaction and an authorization lock; revalidates current source, claim, and thesis context; reuses exact or concurrently-created matching provenance claims; inserts only accepted new main_claims rows at draft status; reuses or inserts only accepted claim_thesis_mappings; inserts one research_publication_recorded journal_entries audit; and returns exact created/reused dispositions and write counts. Exact retries return the recorded result; changed authorization bytes or conflicting provenance/relationship state are refused.
