...
- The recorder uses one serializable transaction and an authorization lock; revalidates current source, claim, and thesis context; retries serialization or uniqueness races with a fresh snapshot; reuses exact or concurrently-created matching provenance claims; inserts only accepted new main_claims rows at draft status; reuses or inserts only accepted claim_thesis_mappings; inserts one research_publication_recorded journal_entries audit containing the validated authorization and immutable prepared envelope; and returns exact created/reused dispositions and write counts. Canonically identical JSON retries return the recorded result; changed canonical authorization content or conflicting provenance/relationship state is refused.
...
9. The recorder is the sole mutation boundary. Its serializable transaction may reuse or insert accepted `main_claims` rows, reuse or insert accepted `claim_thesis_mappings`, and insert one `research_publication_recorded` `journal_entries` audit containing the validated authorization and immutable prepared envelope. It revalidates current state, retries concurrency races with a fresh snapshot, refuses incompatible conflicts or drift, returns exact created/reused dispositions, and rolls back partial failure.
