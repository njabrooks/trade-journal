...
- The recorder uses one serializable transaction and an authorization lock; deterministically re-derives the prepared candidates; revalidates current source, claim, and thesis context; retries serialization or uniqueness races with a fresh snapshot; reuses exact or concurrently-created matching provenance claims only for accepted candidates; inserts only accepted new main_claims rows at draft status; reuses or inserts only accepted claim_thesis_mappings; inserts one research_publication_recorded journal_entries audit containing a compact digest-bound snapshot of the validated authorization and accepted publication; and returns exact created/reused dispositions and write counts. Canonically identical JSON retries return the recorded result; changed canonical authorization content or conflicting provenance/relationship state is refused.
