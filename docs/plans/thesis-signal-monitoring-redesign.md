...
`/configure-signal` is a Claude Code skill — the investigative workflow (trying endpoints, evaluating data quality, reasoning about threshold values) happens as a back-and-forth dialogue in VS Code / Claude Code. Once the config is agreed, the skill writes `explicit_details` to the DB and the signal immediately appears with a working progress bar in the front end. TWO-123 tracks adding a `journal_entry` to capture the configuration rationale.
