...
#### Execution-quality / excursion axis (IMPLEMENTED 2026-06-24 — build note in §9)

The retrospective was answering **one** question — *was the belief right?* — over final P&L, duration, the closing digest, the signal tally, and the journal-trail size. That conflates the belief with how it was traded, so it is now split into **two distinct axes**:

1. **Belief correctness** — was the thesis right? (`outcome` ∈ validated/invalidated/partial — unchanged).
2. **Execution quality** — did we *capture* the P&L that was actually available? Compare P&L at close against the **peak** cumulative P&L during the hold (**maximum favorable excursion, MFE**) and the **worst** cumulative drawdown (**maximum adverse excursion, MAE**), via a capture ratio (`final / MFE`). A thesis that closed at a loss but spent real time meaningfully in profit is an **exit-timing / execution** lesson — distinct from whether the belief was sound; symmetrically a winner that gave back most of its peak is a correct belief, poorly harvested. The two diverge sharply in practice (e.g. PLTR closed at its +$69k peak → execution `excellent`; CVX peaked +$26k and closed −$0.3k → `gave back the move`), and collapsing them into one "outcome" hides the lesson.

**The building block was already there:** `gatherRetrospectiveContext` already loaded the daily P&L series (`perf.combined` from `thesisPerformance.ts`) and scanned it for the open/close dates. MFE/MAE are the peak/trough of `combined[].cumulative` over that same series — a pure scan (`computeExcursion`, `src/lib/derived/retrospectiveExcursion.ts`), no new pipeline. Numbers are computed live everywhere; only the `executionQuality` judgment (+ a frozen metrics snapshot) is persisted (`retrospective_metrics` jsonb).

**Where it surfaces:** the two-axis read feeds the retrospective narrative (`/thesis-review` retrospective mode → `record-retrospective.ts`), the `/performance` `RetrospectiveCard` (Belief + Execution badges + an MFE/capture/MAE strip), and a per-thesis **`RetrospectivePanel`** on the overview pages — an annotated excursion chart (MFE/MAE + event markers) plus a **process timeline** that fuses the four trade-process artefacts onto the curve: signal verdict flips, advisor recs (taken/not), conviction re-underwrites, and raised decisions. It also renders a **`contributors`** attribution breakdown — per-strategy for an asset thesis, per-linked-asset-thesis for a macro (full-credit, D8) — answering *which leg carried the result* at a macro level otherwise two layers removed from execution. MFE/MAE inherit the **`realized_confidence` caveat** (`ConfidenceBadge`) — a non-`full` series is a partial view, never ground truth.

#### Future enhancement (not built): episodic performance — per-expression-period retrospectives

`closed` is **reversible** (closed ⇄ monitoring — a thesis re-expresses automatically when a strategy reappears), but the current retrospective is **one-per-thesis**: `findThesesNeedingRetrospective` excludes any thesis that already has a retrospective entry, and the excursion spans the *entire* P&L series. So a thesis that is held, closed, then re-expressed months later conflates two distinct holding periods into one curve and never re-assesses the second.

The fix is to treat each open→close as an **expression episode** and make performance a short *series* keyed by date range, not a single lifetime record: window the excursion to `[episodeOpen, episodeClose]` (slice `perf.combined`) and write one retrospective per episode. This cleanly subsumes the one-and-done caveat — the worklist keys on "resolved since the last episode's retrospective", the prior episode's frozen metrics persist, and the new episode tracks live from its reopen.

- **Mostly reconstructable already:** the cascade journals every `→ monitoring` and `monitoring → closed` transition (`status_change`, `source=automation`), so episode boundaries are derivable from the journal trail with no new bookkeeping — windowing is the only new logic. A `thesis_expression_episodes` table (`thesis_id`, `episode_no`, `opened_at`, `closed_at`, frozen `retrospective_metrics`, `outcome`, `executionQuality`, narrative) is the heavier-but-cleaner alternative.
- **UI:** card/panel show the latest episode by default + a small "prior episodes" history.
- **Adjacent gap (separate scoping):** there is **no macro creation/suggestion mechanism** — macros are hand-authored (`scripts/ops/create-macro-thesis.ts`, the convert-claim-to-entity dialog); `/thesis-review` framing only *links* assets to *existing* macros, and the strategy→thesis auto-linker creates placeholder *asset* theses, never macros. Anticipatory macro suggestion (cluster linked asset theses / claims into a proposed macro) is the unbuilt **`cluster_claims_to_thesis`** decision from [09](09-claim-signal-propagation-operating-model.md) — a W8 research-redesign item. Episodic tracking matters more once macros come and go dynamically rather than being long-lived hand-authored beliefs.

> **Both pathways are scoped in [13-macro-emergence-and-episodic-performance.md](13-macro-emergence-and-episodic-performance.md)** — episodic performance as an independent 07 follow-on; macro emergence folded into W8.
...
**Signal batch — deferred + spaced (token cost).** 30 monitoring theses still need signals + 9 are thin (research-gap, B6). Rather than one big agent fan-out (~55–65K tokens/agent × 30 ≈ 1.8M), the batch is spaced: process a few per run, ideally via a **scheduled `/thesis-review` routine** (the natural "operationalize B4/B5" step — like `/thesis-monitor`). When built, that routine drains the signal worklist, refreshes delta-triggered digests, and runs the health pass incrementally, keeping per-run token cost bounded. Until then, run `/thesis-review` manually. Sub-agents inherit the session model: belief-loop judgment (digest/signal derivation, health, framing) is **all-Opus — never Sonnet** (settled policy, commit ce5fbae); bound cost by spacing the batch, not by downgrading the model.

**Execution-quality / excursion retrospective (2026-06-24 — §4d two-axis).** The retrospective gained a second axis — *did we capture the P&L that was available?* — alongside the belief verdict. Pure `computeExcursion` (`src/lib/derived/retrospectiveExcursion.ts`, **11 golden tests**) scans the already-loaded `perf.combined` for MFE (peak), MAE (trough), capture ratio, and give-back — no new pipeline. `src/db/queries/retrospectiveView.ts` adds `getRetrospectiveView` + `assembleRetrospectiveEvents`, which fuses **four trade-process artefacts** onto the curve, each snapped to the nearest snapshot date: signal verdict flips (`signal_data_snapshots`, **collapsed to per-signal transitions** — NVDA went 140→8 events), advisor recs taken/not (`advisor_recommendations`), conviction re-underwrites (`thesis_articulations` + confidence deltas), and raised decisions (`decision_required` journal entries). `gatherRetrospectiveContext` now returns `excursion` + `events`; the `/thesis-review` retrospective mode writes both axes and judges `executionQuality` ∈ excellent/good/fair/poor; `record-retrospective.ts` freezes `retrospective_metrics` (new jsonb column on both thesis tables).
- **Design:** numbers are **computed live** everywhere from the one pure function (the post-close series is frozen, so live == frozen); only the `executionQuality` judgment + a metrics snapshot are persisted. The `/performance` grid reads the snapshot and **live-computes a fallback** when null, so every resolved thesis shows the execution axis with **no backfill**. `CLOSED_STATUSES` now includes `closed` (was expressed, now flat → belongs in Retrospectives, not the live tables). MFE/MAE carry the W4/W5 `ConfidenceBadge` caveat.
- **UI:** `ExecutionBadge`/`BeliefBadge`, `RetrospectiveCard` two-axis row + excursion strip, `RetrospectiveExcursionChart` (recharts area + MFE/MAE/event `ReferenceDot`s), and a per-thesis `RetrospectivePanel` (verdict + metric cards + chart + process timeline + narrative) on the asset/macro overview pages when resolved.
- **Validated end-to-end:** 11 excursion tests + full suite (213) green; build green. Recorded PLTR (`validated` / execution `excellent` — closed at its +$69.2k peak, captured 100%, MAE −$170) live through `record-retrospective`; `/performance` renders the live fallback across the book (`captured 100/60/42%`, **`gave back the move` ×24**, `never in profit` ×6 — the CVX archetype: peaked +$26.3k, closed −$0.3k). Execution badges fill in as retrospectives (re-)run; the numbers + timeline already show for all closed theses.
- **Forward-fill correctness fix (from real-data review):** `performanceForStrategyIds.combined` summed only strategies with a snapshot on each exact date, so when a multi-strategy thesis's legs closed at different times the closed legs **dropped out of the running total** — misstating both the final value and MFE/MAE. (Caught on the "Bearish US Equities Short" macro: its two big futures shorts closed mid-life, making the curve "recover" from −$91k to −$1.5k as the losers left the sum, when the true close was −$99.1k.) `combined` now **forward-fills** each strategy's last cumulative (a closed leg's realized P&L persists), mirroring `ThesisPerformanceChart`'s pivot, so `combined[last].cumulative === totals.latestCumulative`. Single-strategy theses (e.g. PLTR) are unchanged. Every `.combined` consumer is retrospective code, so this is the single fix point.
- **Attribution breakdown (D8 at the retrospective):** `getRetrospectiveView` returns `contributors` — per-strategy for an asset thesis, per-linked-asset-thesis for a macro (full-credit) — sorted by magnitude with a % of gross. The `RetrospectivePanel` renders a "Contribution" bar list, answering *which leg carried the result* at the macro level (two layers removed from execution). E.g. Bearish US Equities: ES −$54k (54%) / MNQ −$44k (44%) / HYG −$1.5k (1%).
