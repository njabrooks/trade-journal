...
| B5 | Auto **qualitative** signal derivation on promotion (4b); decouple promotion from signals in insert-thesis-articulation; thesis-health pass (4c) | M | B4 | **partial** — B5a (decouple) + B5b (signal derivation) DONE 2026-06-18; B5c (health pass) + signal batch + flag-flip pending |
...
- **Remaining (B5c + ops):** the monitoring thesis-health pass (re-assess signals vs routed evidence → DecisionStrip on weakening/invalidation); running the 33-thesis signal batch; then flipping `THESIS_CASCADE_ENABLED` live (user decision).
