...
| B5 | Auto **qualitative** signal derivation on promotion (4b); decouple promotion from signals in insert-thesis-articulation; thesis-health pass (4c) | M | B4 | **build DONE** 2026-06-18 (B5a decouple + B5b signal derivation + B5c health pass); remaining ops: signal batch (33) + flag-flip |
...
**Remaining in B5 (ops, not build):** run the 33-thesis signal batch (signal mode); then flip `THESIS_CASCADE_ENABLED` live (user decision). The health pass then runs on a growing signal-bearing set.
