...
- **W9 intel-router quality audit** — ✅ **DONE 2026-06-24.** Routed `assessment` labels were inverted for *invalidation* signals (signal-centric writers vs a thesis-centric read). Fixed at the source: the deterministic routes (`relate-research` `assessSignal`, the `ingest-world-monitor` fallback) now **abstain to `neutral`** (a keyword match can't read stance), and the only directional author — the `/thesis-monitor` `Score:` — was rewritten **thesis-centric**. Historical rows neutralised (3,559 snapshots + 274 evidence links on invalidation signals) via `scripts/ops/backfill-invalidation-polarity.ts`. The health pass already corrected on read; this removes the misleading hint and de-biases the mechanical surfaces. See memory `signal-assessment-polarity`.
- **Signal polarity-aggregation machinery — v1 residue** (retire/de-emphasize candidate for the W8 signals-surface cleanup): `synthesize-signal-day`'s cumulative `strengthening − weakening` score and the per-signal conviction chart (`/api/signals/[id]/daily-scores`, shown on `/signals/[id]`) were built for v1's quantitative curated-signal tracking. In v2 the thesis-health **verdict is qualitative and Claude-rendered** (`/thesis-review` health → `thesis_health` snapshots), so the numeric conviction line no longer drives belief maintenance. Coherent post-fix, but a cleanup candidate.
