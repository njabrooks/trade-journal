...
| **P1** | **Signal-quality diagnostics** (§10.1) — chronic-neutral + surprise detection over snapshot history → re-underwrite-due triggers. **SPECCED → [15-signal-quality-diagnostics.md](15-signal-quality-diagnostics.md)** | **Keystone of the loop.** Cheap (reads existing history); turns the logger into a learning loop | Data-gated: needs a few weeks of observe history to chew on — *build now, let it accumulate*. Bundle the small "surface `explicit_details`/`collector_tracked` flag in `find-theses-due-observe`" hardening so deferral (§3.3) is deterministic, not inferred |
