...
| W7 portfolio-aware options advisor (D11) | **hedge scenario DONE** 2026-06-12 — engine `scripts/options-advisor.ts` (exposures → chains → put/put-spread candidates + vol context; math only), judgment in `/options-advisor` skill, storage `advisor_recommendations` (batch supersede, 7-day expiry; save via `scripts/ops/save-advisor-recommendations.ts`), surfaced in dashboard ScannerSnapshot + `/api/advisor/recommendations`. First real batch live (GLXY 22%-NAV concentration headline). Remaining scenarios: income / put-entry / opportunistic; post-scan scheduling |
