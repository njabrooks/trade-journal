...
Active: pull-portfolio, portfolio-and-options-mcp, ibkr-quote, analyze-vol-curve, options-advisor, build-core-argument, **thesis** (talk-to-your-thesis, docs/v2/10 D3), **decisions** (pull resolver), relate-research, relate-bookmark, maintenance, thesis-review, thesis-observe, assess-validation-evidence, synthesize-claims, finalize-for-upload, backfill-claims, process-transcript, process-note, and the Registry-locked `research-pipeline` CLI. The former named pipeline skills (`pipeline-status`, stage 1–5, `advance-or-kill`, and `graduate-pipeline-idea`) are protective discovery tombstones that route to governed zero-write stage results; they are not executable persistence procedures. `archived-deep-dive`, `archived-generate-summary`, and `paperclip-backlog` are historical evidence only under `docs/archive/provider-adapters/issue-75/`. **`configure-signal` is a no-read/no-write protective tombstone** — signals are auto-derived by build-core-argument, never configured, per docs/v2/10 §6/§9.
